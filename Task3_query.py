...
if __name__ == '__main__':
    ...
    if search_field == 'POS':
        q_solr = q_solr + q_pos_tag[0][1]
        for i in xrange(1,len(q_pos_tag)):
            q_solr = q_solr + '+' + q_pos_tag[i][1]
        q_solr = q_solr + '&wt=json&df=POS&fl=id,name'
    elif search_field == 'stem':
        q_solr = q_solr + q_stem[0]
        for i in xrange(1, len(q_stem)):
            q_solr = q_solr + '+' + q_stem[i]
        q_solr = q_solr + '&wt=json&df=stem&fl=id,name'
    elif search_field == 'lemma':
        q_solr = q_solr + q_lemma[0]
        for i in xrange(1, len(q_lemma)):
            q_solr = q_solr + '+' + q_lemma[i]
        q_solr = q_solr + '&wt=json&df=lemma&fl=id,name'
    elif search_field == 'hypernym':
        q_solr = q_solr + q_hyper[0]
        for i in xrange(1, len(q_hyper)):
            q_solr = q_solr + '+' + q_hyper[i]
        q_solr = q_solr + '&wt=json&df=hypernym&fl=id,name'
    elif search_field == 'hyponym':
        q_solr = q_solr + q_hypo[0]
        for i in xrange(1, len(q_hypo)):
            q_solr = q_solr + '+' + q_hypo[i]
        q_solr = q_solr + '&wt=json&df=hyponym&fl=id,name'
    elif search_field == 'holo':
        q_solr = q_solr + q_holo[0]
        for i in xrange(1, len(q_holo)):
            q_solr = q_solr + '+' + q_holo[i]
        q_solr = q_solr + '&wt=json&df=holonym&fl=id,name'
    elif search_field == 'meronym':
        q_solr = q_solr + q_mero[0]
        for i in xrange(1, len(q_mero)):
            q_solr = q_solr + '+' + q_mero[i]
        q_solr = q_solr + '&wt=json&df=meronym&fl=id,name'
    print(q_solr)
